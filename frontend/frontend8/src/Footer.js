import React from "react";
import "./Footer.css";

const Footer = () => {
  return (
    <footer>
      <section>
        <ul className="socials">
          <li>
            <a
              href="https://twitter.com/"
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                src="https://www.edesk.com/wp-content/uploads/2023/12/logoTwitter.png"
                alt="Twitter"
              />
            </a>
          </li>
          <li>
            <a
              href="https://www.facebook.com/SamuraiTech"
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                src="data:image/png;base64,"
                alt="Facebook"
              />
            </a>
          </li>
          <li>
            <a
              href="https://www.instagram.com/SamuraiTech"
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                src="https://cdn.icon-icons.com/icons2/2429/PNG/512/instagram_logo_icon_147277.png"
                alt="Instagram"
              />
            </a>
          </li>
        </ul>

        <ul className="links">
          <li>
            <a href="mailto:contacto@samuraitech.com" style={{ color: "white" }}>contacto@samuraitech.com</a>
          </li>
          <li>|</li>
          <li>
            <a href="mailto:ventas@samuraitech.com" style={{ color: "white" }}>ventas@samuraitech.com</a>
          </li>
          <li>|</li>
          <li>
            <a href="tel:+525512345678" style={{ color: "white" }}>(55) 1234-5678</a>
          </li>
          <li>|</li>
          <li>
            <a href="tel:+525512345679" style={{ color: "white" }}>(55) 1234-5679</a>
          </li>
          
        </ul>
        <p className="legal" style={{ color: "white" }}>© 2024 Todos los derechos reservados</p>
      </section>
    </footer>
  );
};

export default Footer;